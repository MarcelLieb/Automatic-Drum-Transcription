...
This file implements the Jamba architecture, as proposed by AI21labs (although others have proposed blending Mamba & attention in the past).
...
class JambaLM(nn.Module):
    ...
    def generate(
        ...
        caches = [
            self.jamba.layers[i].get_empty_cache(batch_size, input_ids.device)
        ...
        for i in range(input_ids.size(1) + max_tokens - 1):
            with torch.no_grad():
                ...
                next_token_logits, caches = self.step(
                    input_ids[:, [i]], caches
                )  #  (batch_size, 1, vocab_size), caches
...
class AttentionSDPA(nn.Module):
    ...
    def forward(self, x, cache=None):
        ...
        values = values.view(B, L, self.num_key_value_heads, self.head_dim).transpose(
            1, 2
        )
...
class MambaLayer(nn.Module):
    ...
    def get_empty_cache(self, batch_size, device):
        return None, torch.zeros(
            batch_size, self.config.d_inner, self.config.d_conv - 1, device=device
        )


class SparseMoEBlock(nn.Module):
    def __init__(
        self, config: JambaLMConfig, num_experts: int, num_experts_per_tok: int
    ):
        super().__init__()
...
def load_balancing_loss(router_logits, num_experts, num_experts_per_tok):
    ...
    # percentage of tokens routed to each expert
